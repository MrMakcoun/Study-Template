...
namespace Study.LabWork2.Feature.Task1.SubTask1
{
    ...
    public sealed class MonitorService : IPrimeCounter
    {
        ...
        private readonly List<int> _FoundPrimes = new(); // если нужно собирать список
        ...
        private void CheckPrimesInRange(int start, int end, int threadId)
        {
            for (int number = start; number <= end; number++)
            {
                ...
                if (number % 100 == 0)
                {
                    Console.WriteLine($"[Monitor] Поток {threadId}: обрабатываемое число {number}");
                }
            }
            Console.WriteLine($"[Monitor] Поток {threadId} завершил работу.");
        }
        ...
        /// </summary>
        /// <returns>Строка "Monitor (lock)"</returns>
