    ...
    public sealed class SemaphoreService : IPrimeCounter
    {
        ...
        private void CheckPrimesInRange(int start, int end, int threadId)
        {
            for (int number = start; number <= end; number++)
            {
                ...
                if (number % 100 == 0)
                {
                    Console.WriteLine($"[Semaphore] Поток {threadId}: обрабатываемое число {number}");
                }
            }
            Console.WriteLine($"[Semaphore] Поток {threadId} завершил работу.");
        }
        ...
        /// </summary>
        /// <returns>Строка "Semaphore"</returns>
